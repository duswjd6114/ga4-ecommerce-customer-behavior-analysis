...
이 프로젝트는 Google Merchandise Store의 GA4 공개 샘플 데이터를 활용하여 사용자의 방문부터 구매까지의 행동을 분석한 개인 프로젝트입니다. BigQuery SQL로 이벤트 로그를 세션·사용자·거래 단위로 가공하고, 구매 퍼널·코호트 리텐션·디바이스·상품·반복 구매 분석을 통해 주요 이탈 구간과 개선 과제를 도출했습니다.

추가로 별도의 이커머스 랜딩페이지 실험 데이터를 활용하여 전환율 A/B 테스트를 수행했습니다. GA4 분석에서 발견한 문제와 A/B 테스트 데이터는 직접 연결된 동일 실험이 아니며, A/B 테스트는 통계적 검정 역량을 보여주기 위한 별도 분석입니다.
...
- 별도 실험 데이터를 활용한 전환율 A/B 테스트 및 의사결정
...
- **환경**: Google BigQuery, Google Colab
...
- `user_pseudo_id`와 `event_name` 결측은 확인되지 않았습니다.
...
- 방문 세션 중 구매까지 도달한 비율은 약 **0.79%**였습니다.
- 상품 조회 후 장바구니 전환율이 **19.79%**로 가장 낮아 핵심 개선 구간으로 판단했습니다.
- 절대 이탈 세션은 방문 후 상품 조회 구간에서 가장 많았습니다.

![5단계 구매 퍼널](visualizations/01_funnel_chart.png)

### 7.2 디바이스별 퍼널

- 전체 구매 전환율은 모바일 0.82%, 태블릿 0.79%, 데스크톱 0.77%였습니다.
- 기기별 차이는 작았으며, 모든 기기에서 상품 조회 후 장바구니 구간이 공통 병목으로 나타났습니다.
- 특정 기기보다 상품 상세 정보와 장바구니 유도 요소를 우선 개선할 필요가 있습니다.

![디바이스별 퍼널 전환율](visualizations/02_device_funnel_rates.png)
...
![주차별 코호트 리텐션](visualizations/03_cohort_retention_heatmap.png)
...
- 3개월만으로 계절성이나 프로모션 효과를 단정하지 않았습니다.

![월별 구매 성과](visualizations/04_monthly_purchase_performance.png)

### 7.5 상품별 매출

- `Google Zip Hoodie F/C`가 $10,488로 가장 높은 매출을 기록했습니다.
- `Super G Unisex Joggers`는 주문·판매수량은 많았지만 매출 순위는 5위로, 상품별 가격 구조 차이를 확인했습니다.
- 판매수량뿐 아니라 매출과 주문 수를 함께 고려해 상품 성과를 해석했습니다.

![매출 상위 상품](visualizations/05_top_products_by_revenue.png)

### 7.6 반복 구매

- 유효 거래번호 기준 구매 사용자는 3,702명이었습니다.
- 일회 구매자는 3,263명(88.14%), 반복 구매자는 439명(11.86%)이었습니다.
- 구매자의 대부분이 한 번만 주문해 첫 구매 이후 재구매 전환이 개선 과제로 나타났습니다.

![반복 구매 비율](visualizations/06_repeat_purchase_share.png)
...
p-value가 0.05보다 크고 신뢰구간에 0이 포함되므로, 새 페이지가 전환율을 높였다는 통계적 증거를 확인하지 못했습니다. 따라서 새 페이지를 전면 적용하지 않고 기존 페이지를 유지한 뒤 개선안을 재설계하여 다시 실험하는 것이 적절하다고 판단했습니다.

![A/B 테스트 전환율](visualizations/07_ab_conversion_rate.png)
...
```text
ga4_ecommerce_customer_behavior_analysis/
├── raw_data/          # A/B 테스트 원본 데이터
├── sql/               # BigQuery SQL
├── notebooks/         # GA4 시각화 및 A/B 테스트 노트북
├── data_outputs/      # 분석 결과 CSV
├── visualizations/    # 분석 그래프
└── screenshots/       # 분석 과정 증빙 화면
```
...
- Python을 활용한 데이터 검증·시각화
- 2표본 비율 Z검정과 신뢰구간 기반 의사결정
